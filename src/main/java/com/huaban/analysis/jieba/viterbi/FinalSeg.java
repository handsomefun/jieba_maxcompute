...
public class FinalSeg {
    ...
    private FinalSeg(InputStream ins){
        is = ins;
    }
    ...
    public synchronized static FinalSeg getInstance(InputStream inputStream) {
        if (null == singleInstance) {
            singleInstance = new FinalSeg(inputStream);
            singleInstance.loadModel();
        }
        return singleInstance;
    }
    ...
    private void loadModel() {
        ...
        try {
            ...
            System.err.println(String.format(Locale.getDefault(), "%s: load model failure!"));
        }
        finally {
            try {
                if (null != is)
