...
public class WordDictionary {
    ...
    private static InputStream mainIns;
    private static InputStream userIns;
    ...
    public static WordDictionary getInstance(InputStream mainDictInputStream, InputStream userDictInputStream) {
        if (singleton == null) {
            synchronized (WordDictionary.class) {
                if (singleton == null) {
                    singleton = new WordDictionary(mainDictInputStream, userDictInputStream);
                    return singleton;
                }
            }
        }
...
//    }
    ...
    public void loadUserDict(Charset charset) {
        try {
            BufferedReader br = new BufferedReader(new InputStreamReader(userIns, Charset.forName("UTF-8")));
            long s = System.currentTimeMillis();
            int count = 0;
            while (br.ready()) {
                String line = br.readLine();
            ...
            System.out.println(String.format(Locale.getDefault(), "user dict %s load finished, tot words:%d, time elapsed:%dms", "userdict", count, System.currentTimeMillis() - s));
            br.close();
        }
        catch (IOException e) {
            System.err.println(String.format(Locale.getDefault(), "%s: load user dict failure!", "usedict"));
        }
    }
